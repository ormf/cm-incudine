...
;;; generated by scheme->cltl from utils.scm on 03-May-2011 10:30:31
...
(defun quote-if-necessary (x)
  (if (or (numberp x) (stringp x) (vectorp x)
...
(defun cm-version-name ()
  (format nil "~a.~a.~a" (ldb (byte 8 16) %cm-version%)
      ...
      (setf pos2
              (or
               (do ((i pos1 (+ i 1))
                    (f nil))
                   ((or f (= i end)) f)
                 (if (member (elt string i) delimiters)
                     (setf f i)))
               end))
      (unless (= pos1 pos2)
        (rplacd tail
