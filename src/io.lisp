...
;;; generated by scheme->cltl from io.scm on 04-Aug-2009 14:11:46
...
(progn (defclass rt-stream (event-stream)
...
(progn (defclass event-file (event-stream)
...
(defmethod init-io ((io event-stream) &rest inits)
  (unless (null inits)
    (multiple-value-bind (init args)
      ...
      (if (not (null args)) (setf (event-stream-args io) args))))
  io)
...
(defmethod open-io ((obj event-file) dir &rest args)
  args
...
(defmethod close-io ((io event-file) &rest mode)
  mode
  (when (io-open io)
...
(defun events (object &rest args)
  (let* ((to
    ...
    (if (or err? (not *out*))
	    ...
	    ;; tito: modificato il path per introdurre la directory di default
            (let* ((path-old (file-output-filename *out*))
		   (path (concatenate 'string
				      (or
				       (filename-directory path-old)
				       (pwd))
				      path-old))
...
(defmethod stream-receive-init ((stream rt-stream) hook args)
  stream
  hook
  args
  (values))
...
(defmethod stream-receive-start ((stream rt-stream) args)
  (let* ((type (rt-stream-receive-type stream))
    ...
    (if (not meth)
        nil
        (let ((start (cadr meth))) (funcall start stream args)))))
...
(defmethod stream-receive-stop ((stream rt-stream))
  (let* ((type (rt-stream-receive-type stream))
    ...
    (if (not meth)
        nil
        (let ((stop (caddr meth))) (funcall stop stream)))))
  ...
  (let ((data (rt-stream-receive-data stream)))
    (if (and (not (null data)) (first data))
        ...
        (let ((type (getf args ':receive-type)))
          (if type
              (setf (rt-stream-receive-type stream) type)
          ...
          (cond ((stream-receive-start stream args)
                ...
                (t
                 ...
                 (error "set-receiver!: ~s does not support :receive-type ~s."
                        stream
