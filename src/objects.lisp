;;; **********************************************************************
;;; Copyright (C) 2011 Heinrich Taube, <taube (at) uiuc (dot) edu>
;;;
...
;;; generated by scheme->cltl from objects.scm on 28-Dec-2011 13:48:16
...
(defmethod make-load-form ((obj container))
  `(make-instance
    ,(intern
      (string-upcase (format nil "<~a>" (class-name (class-of obj))))
      :cm)
    ,@(slot-init-forms obj :eval t :omit '(subobjects)) :subobjects
    ,(cons 'list
  ...
  (let* ((err?
    ...
    (if old
        (if (eq obj old)
            old
            (if err?
                (error "The name ~a already references ~s." newname
                       old)
                nil))
        (progn
         (remhash (object-name obj) *dictionary*)
         (setf (object-name obj) str)
         (setf (gethash (string-downcase str) *dictionary*) obj)
         obj))))
...
(defun find-object (string &optional err? class)
  (let* ((name
               ...
               (let* ((key (object-name val))
                 ...
                 (if typ
                     (let ((nam (filename-name key))
                       ...
                       (if (and (string= type typ) (string= name nam)
    ...
    (or find
        (if err?
            (error "No object named ~s." string)
            nil))))
...
(defun map-objects
       (fn objs
        ...
        &aux doat indx this)
  (if (not (listp objs))
      (setf objs (container-subobjects objs)))
  (if (and slot slot!)
