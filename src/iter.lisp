...
;;; generated by scheme->cltl from loop.scm on 28-Dec-2011 13:48:16
  ...
  (let ((var (cadr forms))
    ...
    (do ((next nil))
      ...
      (case next
        ...
        ((by)
         (if step
             (loop-error ops forms "Found duplicate 'by'."))
         (setf step (pop tail)))
        (t
...
(defun legal-in-conditional? (x ops)
  (let ((op (loop-op? x ops)))
    (if (and op (not (null (cddr op))) (eq (caddr op) 'task)
